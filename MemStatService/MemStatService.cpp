...
void MyServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	//CloseHandle(hFile);
	hFile = CreateFile("C:\\LocalTimeLog.txt", GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	CloseHandle(hFile);
	// 서비스가 시작되었음을 알린다.
...
void MyServiceHandler(DWORD fdwControl)
{
	...
	case SERVICE_CONTROL_NEWFILE:
		hFile=CreateFile("C:\\LocalTimeLog.txt",GENERIC_WRITE,0,NULL,
			CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
		lstrcpy(gbuf,"시스템 시간 로그\r\n");
		CloseHandle(hFile);
